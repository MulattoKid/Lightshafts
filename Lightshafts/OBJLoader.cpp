...
void LoadOBJ(const std::string& file, Mesh* mesh)
{
	...
	int mesh_details[5] = { 0, 0, 0, 0, 0 }; //[0]=num_vertices, [1]=num_uvs, [2]=num_normals, [3]=num_triangle_faces, [4]=num_quads_faces
	CountMeshDetails(reader, mesh_details);
	assert(mesh_details[4] == 0); //Make sure mesh has no quads
	...
	unsigned int triangle_faces_vertices_index = 0, triangle_faces_uvs_index = 0, triangle_faces_normals_index = 0, triangle_faces_mtls_index = 0;
    ...
    //Create triangles
	mesh->num_triangles = mesh_details[3];
	if (mesh->num_triangles > 0)
	{
		...
		{
			Triangle t;
			t.v0 = vertices[triangle_faces_vertices[i]];
			t.v1 = vertices[triangle_faces_vertices[i + 1]];
			t.v2 = vertices[triangle_faces_vertices[i + 2]];
			if (has_uvs)
			{
				t.uv0 = uvs[triangle_faces_uvs[i]];
				t.uv1 = uvs[triangle_faces_uvs[i + 1]];
				t.uv2 = uvs[triangle_faces_uvs[i + 2]];
			}
			if (has_normals)
			{
				t.normal0 = normals[triangle_faces_normals[i]];
				t.normal1 = normals[triangle_faces_normals[i + 1]];
				t.normal2 = normals[triangle_faces_normals[i + 2]];
			}
			glm::vec3 v0v1 = t.v1 - t.v0;
			glm::vec3 v0v2 = t.v2 - t.v0;
			t.normal = glm::normalize(glm::cross(v0v1, v0v2));

			i += 3;
			mesh->triangles[t_i++] = t;
		}
	}
